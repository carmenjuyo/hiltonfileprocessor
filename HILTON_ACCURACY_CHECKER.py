...
def load_files(csv_path=None, operational_report_path=None, ideas_report_path=None):
    # Check which files have been provided
    csv_loaded = csv_path is not None
    operational_report_loaded = operational_report_path is not None
    ideas_report_loaded = ideas_report_path is not None

    # Logic based on file availability
    if csv_loaded and operational_report_loaded and not ideas_report_loaded:
        # Show past data variance table, past accuracy graph, and summary
        show_past_data_variance()
        show_past_accuracy_graph()
        show_past_summary()
    elif csv_loaded and ideas_report_loaded and not operational_report_loaded:
        # Show future variance table, future accuracy graph, and summary
        show_future_data_variance()
        show_future_accuracy_graph()
        show_future_summary()
    elif csv_loaded and operational_report_loaded and ideas_report_loaded:
        # Show both past and future data variance, accuracy graphs, and summaries
        show_past_data_variance()
        show_past_accuracy_graph()
        show_past_summary()
